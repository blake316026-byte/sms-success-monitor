...
21. 当日财务数据优先读取当前后台 origin 下的只读 `POST /api/dashboard4bix/realtime`，严格使用 `model.today.rechargeSuccAmount` 和 `model.today.withdrawSuccAmount` 等今日汇总位置，不得从图表、小时趋势或明细候选中挑金额。sixsass/OK01 使用只读 `POST /api/realtime_record/with_country`、请求体 `{dayOffset: 0, countryId: "PH"}`，从响应中的 `rechargeAmount` 和 `withdrawAmount` 读取，允许金额字段包在 `data`、`model` 等嵌套对象中。若响应里同时存在图表/小时明细和今日汇总金额，必须优先选择今日汇总金额，不得取第一条明细。今日充提差统一由客户端按“充值金额 - 提现金额”计算。
...
| macOS | `0.3.27 (31)` |
| Windows | `0.3.27` |
| Android | `versionName 0.3.27` / `versionCode 31` |
| 开发仓库分支 | `feat/standalone-sms-success-monitor` |
| Git 提交 | 发布前通过开发分支 HEAD 与公开仓库 `v0.3.27` 标签现场核验 |
| GitHub Release | `https://github.com/blake316026-byte/sms-success-monitor/releases/tag/v0.3.27` |
| 本机 macOS 安装 | `/Applications/SMS Success Monitor.app`，目标 `0.3.27 (31)` |
