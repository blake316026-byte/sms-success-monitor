...
21. 当日财务数据源为当前后台 origin 下的只读 `POST /api/dashboard4bix/realtime`。字段固定读取 `model.today.rechargeSuccAmount`（今日充值金额）和 `model.today.withdrawSuccAmount`（今日提现金额），今日充提差由客户端按“充值金额 - 提现金额”计算。
...
| macOS | `0.3.22 (26)` |
| Windows | `0.3.22` |
| Android | `versionName 0.3.22` / `versionCode 26` |
| 开发仓库分支 | `feat/standalone-sms-success-monitor` |
| Git 提交 | 通过开发分支 HEAD 与公开仓库 `v0.3.22` 标签现场核验 |
| GitHub Release | `https://github.com/blake316026-byte/sms-success-monitor/releases/tag/v0.3.22` |
| 本机 macOS 安装 | `/Applications/SMS Success Monitor.app`，目标 `0.3.22 (26)` |
