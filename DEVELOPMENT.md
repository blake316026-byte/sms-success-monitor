...
21. 当日财务接口必须按平台严格分流，不允许失败后跨平台探测接口。普通 NPG 后台只读取当前 origin 下的只读 `POST /api/dashboard4bix/realtime`，严格使用 `model.today.rechargeSuccAmount` 和 `model.today.withdrawSuccAmount`。OK01 只使用只读 `POST /api/realtime_record/with_country`、请求体 `{dayOffset: 0, countryId: "PH"}`，并发送 `COUNTRY: PH` 与 `SYSTIMEZONE: Asia/Manila`；金额必须取响应 `list` 中 `countryId=PH`、`type=COUNTRY_DAY`、`timeType=DAY` 且无 `appId` 的汇总行，从 `columns.rechargeAmount` 和 `columns.withdrawAmount` 读取菲律宾比索，不得读取小时行或 APP 行。OK01 绝不请求 `/api/dashboard4bix/realtime`。财务数据由独立的 20 秒任务刷新，短信扫描不得附带财务请求。今日充提差统一由客户端按“充值金额 - 提现金额”计算。
...
| macOS | `0.3.36 (40)` |
| Windows | `0.3.36` |
| Android | `versionName 0.3.36` / `versionCode 40` |
| 开发仓库分支 | `feat/standalone-sms-success-monitor` |
| Git 提交 | 发布前通过开发分支 HEAD 与公开仓库 `v0.3.30` 标签现场核验 |
| GitHub Release | `https://github.com/blake316026-byte/sms-success-monitor/releases/tag/v0.3.36` |
| 本机 macOS 安装 | `/Applications/SMS Success Monitor.app`，目标 `0.3.36 (40)` |
...
本轮发布已通过项目全量测试、macOS Universal 2 打包与本地 OCR/TOTP/page-find 自检、Windows x64 交叉打包、Android Lint 与 APK 构建。已在本机 macOS 安装并验证客户端从 `/Applications` 启动；Windows 10/11、Android 8.0+ 实机自动登录和 OKBET 真实账号扫描仍未验证，不能据此宣称三端实机验收完成。

### 8.2 OK01 接入契约

`OK01` 已加入第十个固定后台，三端配置 ID 为 `ok01`，默认工作台地址为 `/sms-record-list`，macOS 使用独立持久化会话 UUID `53D12001-4A6B-4C00-9000-000000000301`。公开 Release 不内置真实后台域名；macOS 真实后台地址从用户本机 `~/Library/Application Support/SMS Success Monitor/modules-v1.json` 读取。
...
- 短信扫描请求体使用 `query.pageNo`、`query.pageSize: 20` 和 `query.ddCreated`；`SENT` 为发送中，`SUCCESS` 为成功；
- 今日统计使用 `POST /api/realtime_record/with_country`、请求体 `{dayOffset: 0, countryId: "PH"}`，`rechargeAmount` 为今日充值金额，`withdrawAmount` 为今日提现金额；
