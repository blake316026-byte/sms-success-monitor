...
当前正式版本：**v0.3.36**（macOS build 40、Android versionCode 40）。本版将短信扫描与财务刷新彻底分离：短信只读取有界分页样本，财务按平台调用汇总接口；OKBET 精确读取菲律宾 `COUNTRY_DAY` 汇总金额，并兼容本机新增的 `okbet` 自定义标签。公开 Release 仍不内置真实后台链接、登录凭据或 Token。正式发布页：[SMS Success Monitor v0.3.36](https://github.com/blake316026-byte/sms-success-monitor/releases/tag/v0.3.36)。
...
各设备的登录态互不同步，需要在每台设备分别登录。固定监控标签不可删除；自定义页面可配置自动登录，并自动加入成功率统计，重命名、修改地址或删除后会同步更新监控总览。

样本条数也是每台设备独立保存，不会上传数据库。修改后旧统计会清空，客户端按新样本数完成扫描后再恢复正常或报警状态。

首次启用自动登录：选择对应后台，点击钥匙图标，输入账号、密码和可选 Google 密钥并保存。每个后台独立配置；图片验证码最多连续尝试 10 次，Google 验证码最多尝试 5 次，分别达到上限后才暂停自动登录并提示人工处理。

客户端会优先复用仍然有效的本机 Token。若网页残留的旧 Token 已失效，会继续尝试本机加密保存的 Token；有效时恢复网页会话，无可恢复会话或 Token 真正失效时才使用账号资料自动登录。

对于入口地址本身是登录页的后台，客户端会先用有效 Token 打开短信记录页；如果网页仍返回登录页，则立即启动图形验证码自动登录，不会只显示正常监控数据而把网页留在登录页等待。
