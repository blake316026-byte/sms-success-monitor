...
各设备的登录态互不同步，需要在每台设备分别登录。固定监控标签不可删除，自定义页面不加入固定成功率统计。
...
macOS 包为本机临时签名，Windows 包未购买代码签名证书，Android 包使用本机调试证书签名，均适合内部使用。系统首次打开时可能显示来源提示；正式公开分发前应改用 Apple Developer、Windows Authenticode 和长期 Android 发布证书签名。

## 安装

macOS：解压后把 `SMS Success Monitor.app` 放入“应用程序”，再打开。重启电脑后可在“应用程序”中再次启动。

Windows：把 ZIP 完整解压到固定目录，运行 `SMS Success Monitor.exe`。不要只把 EXE 单独拖出文件夹。
...
```bash
cd /Users/lucky/Documents/Codex/2026-07-11/sms-success-monitor/standalone/sms-success-monitor
